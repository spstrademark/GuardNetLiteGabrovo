 ...
 */
package org.tensorflow.lite.examples.posenet.Posenet
...
class Posenet(
  ...
  private var myHashMap = HashMap<Int, Any>()
  /** A ByteBuffer to hold image data, to be feed into Tensorflow Lite as inputs.  */
  protected var imgData: ByteBuffer? = null
  /* Preallocated buffers for storing image data in. */
  private val imageSizeX = 257
  private val imageSizeY = 257
...
//   */
  private fun initInputArray(bitmap: Bitmap): ByteBuffer {
    ...
    var size = batchSize * bytesPerChannel * bitmap.height * bitmap.width * inputChannels
...
//            convertBitmapToByteBuffer(bitmap)
   ...
   // val inputArray = arrayOf(imgData)
    ...
    {
      this.myHashMap =     initOutputMap(getInterpreter())
    }


    val outputMap = this.myHashMap//initOutputMap(getInterpreter())
    ...
    objects.keyPoints = keypointList.toList()
    objects.score = totalScore / numKeypoints
    val tEnd = SystemClock.elapsedRealtimeNanos()
    val tDelta: Long = tEnd - tStart
    val elapsedSeconds = tDelta.toDouble()
